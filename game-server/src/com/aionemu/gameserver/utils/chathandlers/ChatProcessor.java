...
public class ChatProcessor implements GameEngine {
	...
	public void init() {
		...
		log.info("加载了 " + commandHandlers.size() + " 条聊天命令.");
	}
	...
	public void registerCommand(ChatCommand cmd) {
		if (cmd.getLevel() < 0)
			throw new NullPointerException("注册聊天命令失败: " + cmd.getAlias() + " 的访问级别无效.");
		if (commandHandlers.putIfAbsent(cmd.getAlias().toLowerCase(), cmd) != null)
			throw new IllegalArgumentException("注册聊天命令失败: " + cmd.getAlias() + " 已经被注册.");
		log.info("注册了聊天命令: " + cmd.getAlias());
	}
	...
	public void handleConsoleCommand(Player player, String text) {
		...
		if (cmd == null)
			PacketSendUtility.sendMessage(player, "命令 " + cmdName + " 未实现.");
