...
public abstract class ChatCommand {
	...
	public final boolean run(Player player, String... params) {
		if (params.length == 1 && "help".equalsIgnoreCase(params[0])) {
			sendMessagePackets(player, "命令: " + ChatUtil.color(getAliasWithPrefix(), Color.WHITE) + "\n\t"
				+ (getDescription().isEmpty() ? "没有可用的描述." : getDescription()) + "\n" + getSyntaxInfo());
			return true;
		}
		...
		try {
			...
			log.error("执行聊天命令 \"" + getAliasWithPrefix() + " " + String.join(" ", params) + "\" 时出错 - 玩家: " + player.getName()
				+ ", 目标: " + player.getTarget(), t);
			return false;
		}
		return true;
	}
	...
	public final byte getLevel() {
		...
		if (level == null)
			throw new NullPointerException("缺少 " + getAliasWithPrefix() + " 的访问级别");
		return level;
	}
