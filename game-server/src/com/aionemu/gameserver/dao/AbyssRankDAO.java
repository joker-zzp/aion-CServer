...
public class AbyssRankDAO {
	...
	// 首先添加AP排名专用的SQL语句
	private static final String SELECT_RANKING_LIST_PLAYERS_AP = "SELECT a.rank_pos, a.player_id, a.ap FROM abyss_rank a, players p WHERE a.player_id = p.id AND p.race = ? AND a.rank_pos > 0 ORDER by a.rank_pos";
	private static final String UPDATE_RANKING_LIST_PLAYERS_POSITIONS_AP = "UPDATE abyss_rank SET rank_pos = @a:=@a+1 WHERE ap > 0 AND player_id IN (SELECT id FROM players WHERE race = ? AND (@minLastOnline IS NULL OR last_online >= @minLastOnline)) ORDER BY ap DESC LIMIT ?";

	public static List<RankingListPlayerGp> loadRankingListPlayersGp(Race race) {
		try (Connection con = DatabaseFactory.getConnection();
				PreparedStatement stmt = con.prepareStatement(RankingConfig.isApOnlyMode() ?
					SELECT_RANKING_LIST_PLAYERS_AP : SELECT_RANKING_LIST_PLAYERS_GP)) {
			...
			while (rs.next()) {
				...
				// 根据排名模式选择读取AP或GP
				int value = RankingConfig.isApOnlyMode() ? rs.getInt("ap") : rs.getInt("gp");
				rankingList.add(new RankingListPlayerGp(rankPos, playerId, value));
			}
			return rankingList;
		} catch (SQLException e) {
	...
	public static void updateRankingLists(int maxOfflineDays, int playerLimit, int legionLimit) {
		try (Connection con = DatabaseFactory.getConnection()) {
			...
			// 根据排名模式选择不同的SQL语句
			String updatePlayersSql = RankingConfig.isApOnlyMode() ? 
				UPDATE_RANKING_LIST_PLAYERS_POSITIONS_AP : UPDATE_RANKING_LIST_PLAYERS_POSITIONS;

			try (PreparedStatement stmt = con.prepareStatement(updatePlayersSql)) {
				if (maxOfflineDays > 0)
