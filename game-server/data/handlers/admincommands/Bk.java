...
public class Bk extends AdminCommand {
  ...
  public void execute(Player player, String... params) {
    if (params == null || params.length < 1) {
      PacketSendUtility.sendMessage(player, "用法 //bk <add|del|tele|list>");
      return;
    }
    ...
    if (params[0].equals("add"))
      try {
        ...
        if (isBookmarkExists(bookmark_name, player.getObjectId())) {
          PacketSendUtility.sendMessage(player, "书签 " + bookmark_name + " 已存在！");
          return;
        }
        ...
        PacketSendUtility.sendMessage(player, "书签 " + bookmark_name + " 已成功添加到您的书签列表！");
      ...
      try {
        ...
        PacketSendUtility.sendMessage(player, "书签 " + bookmark_name + " 已成功从您的书签列表中移除！");
        updateInfo(player.getObjectId());
      }
      ...
      try {
        ...
        try {
          ...
          if (tele_bk != null) {
            ...
            PacketSendUtility.sendMessage(player, "已传送到书签 " + tele_bk.getName() + " 位置");
          }
        }
      } catch (Exception e) {
        PacketSendUtility.sendMessage(player, "用法 //bk <add|del|tele> <书签名称>");
      ...
      updateInfo(player.getObjectId());
      PacketSendUtility.sendMessage(player, "=====书签列表开始=====");
      ...
      PacketSendUtility.sendMessage(player, "=====书签列表结束=======");
    }
  }
