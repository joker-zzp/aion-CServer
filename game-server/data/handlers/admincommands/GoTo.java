...
public class GoTo extends AdminCommand {
  ...
  public GoTo() {
    super("goto", "通过名称传送到指定区域。");

    // @formatter:off
    setSyntaxInfo(
      " - 显示可传送的位置列表。",
      "<location name> - 传送到指定位置。"
    );
    // @formatter:on
    addLocations();
  }
  ...
  public void execute(Player player, String... params) {
    ...
    if (location == null) {
      ...
      if (isSingleSureMatch(matches, destination)) {
        ...
        StringBuilder msg = new StringBuilder("找不到指定的目的地。");
        if (!matches.isEmpty()) {
          msg.append(" 可能的匹配:");
  ...
  private void listLocations(Player player) {
    ...
    if (locsByWorld.size() == 1)
      sb.append("位置列表 (").append(getName(locsByWorld.keySet().iterator().next())).append("): ");
    else
      sb.append("各地图位置列表:\n");
